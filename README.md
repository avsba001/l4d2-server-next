...
```sh
bash <(curl -sL https://raw.githubusercontent.com/LaoYutang/l4d2-server/master/manifest/install/install.sh)
```
服务器无法连接github也可以下载脚本，手动上传都服务器运行。或者使用github加速，如
```sh
bash <(curl -sL https://gh.dpik.top/https://raw.githubusercontent.com/LaoYutang/l4d2-server/master/manifest/install/install.sh)
```
